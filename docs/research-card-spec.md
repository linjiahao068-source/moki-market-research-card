...
v0.3.7 起，用户界面从多种实验卡片类型收敛为 `Executive Investment View`。v0.4.0 已新增 `ResearchReport` schema 基础层，`ResearchCard` 继续作为 legacy 兼容输入与现有渲染层存在。
...
- `ResearchReport.schemaVersion` 当前固定为 `v0.4.0`。
