...
export function buildResearchReportFromCard(card: ResearchCard): ResearchReport {
  ...
  const reportBase: Omit<ResearchReport, 'evidenceLayer' | 'buySideReport' | 'technicalDashboard' | 'integratedReport'> = {
    ...
    generationState: {
      ...
      nativeJson: false,
      fallbackUsed: true,
      warnings: ['ResearchReport generated from legacy ResearchCard adapter; prefer native /api/research-report output when the API is available.'],
    },
    ...
    evidenceReferences,
    factReferences,
    technicalDataSnapshot: card.technicalDataSnapshot,
    followUpResearch,
